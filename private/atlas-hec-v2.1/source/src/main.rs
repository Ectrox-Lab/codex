...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("⚡ Atlas v2.0 - Multi-step Encoding\n");
    
    const N_NEURONS: usize = 1000;
    const BATCH_SIZE: usize = 256;
    const N_STEPS: usize = 20;  // 20个时间步累积
    ...
    for epoch in 0..N_EPOCHS {
        ...
        for batch_idx in 0..n_batches {
                ...
                .zip(spike_counts.chunks(N_NEURONS))
            {
                let features: Vec<f32> = spike_chunk.iter()
                    .map(|&s| s as f32 / N_STEPS as f32)  // 归一化到[0,1]
                    .collect();
        ...
        println!("\r  Epoch {} Complete | Loss: {:.4} | Train Acc: {:.1}% | Time: {:?}", 
            epoch + 1, avg_loss, train_acc, epoch_time);
        
        // 测试集评估
        let test_acc = evaluate(&mut atlas, &mut mlp, &test_data, &stream, N_NEURONS, N_STEPS)?;
...
fn evaluate(
    ...
    let mut correct = 0usize;
    let eval_batches = n_batches.min(20); // 测20个batch加速
    ...
    let n_eval = (eval_batches * batch_size) as f32;
    Ok(correct as f32 / n_eval * 100.0)
}
